...
free_block(uint32_t blockno)
{
	...
	if ( blockno >= lowest_data_block && blockno < n_blocks) {
		...
		eprintk("Invalid block number %u to be freed. Ignoring request...\n", blockno);
}
...
direct_index(uint32_t b)
{
	...
	if ( b < OSPFS_NDIRECT )
		return b;
	// b fits somewhere in indirect (>= 10 && < 266)
	else if ( b < OSPFS_NDIRECT + OSPFS_NINDIRECT )
		return b - OSPFS_NDIRECT;
	// b fits somewhere in doubly indirect (>= 266)
	else
		return (b - OSPFS_NDIRECT - OSPFS_NINDIRECT) % OSPFS_NINDIRECT;
}
...
// zero_block(blockno)
//   Sets all data in the block to zero
...
add_block(ospfs_inode_t *oi)
{
	...
	// Cannot grow file larger than max file size
	if ( n == OSPFS_MAXFILEBLKS )
		return -EIO;

	// Allocate block because we need at least one block in all cases
	new_block[0] = allocate_block();
...
remove_block(ospfs_inode_t *oi)
{
	...
	// File should not be larger than max file size
	if ( n > OSPFS_MAXFILEBLKS )
		return -EIO;
	...
	// n between direct max (10) and indirect max (266)
		...
		if (!indir_block_data)
			return -EIO;
		// Free direct block inside indirect block data
	...
	// n in doubly indirect (> 266)
		...
		if (!indir2_block_data)
			return -EIO;
		// Get pointer to indirect block data inside doubly indirect
		indir_block_data = (uint32_t*) ospfs_block(indir2_block_data[indir_index(n-1)]);
		if (!indir_block_data)
			return -EIO;
		// Free direct block inside indirect inside doubly indirect block data
		free_block(indir_block_data[direct_index(n-1)]);
		// Set direct block's pointer to 0
		indir_block_data[direct_index(n-1)] = 0;
		blocks_removed++;
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	int retval = 0;
	size_t amount = 0;		// Amount of data already read
	...
	while (amount < count && retval >= 0) {
		...
		if ( n > count - amount )		// Figure out if we actually need to read whole rest of the block
			n = count - amount;			// Limit n bytes to read to just the remaining of count
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	else if ( oi->oi_size < initial_pos + count ) {
	...
	while (amount < count && retval >= 0) {
		...
		off = *f_pos % OSPFS_BLKSIZE;	// Find offset into the specific block
		n = OSPFS_BLKSIZE - off;		// Set n to max possible bytes left in block
		if ( n > count - amount )		// Figure out if we actually need to write whole rest of the block
			n = count - amount;			// Limit n bytes to write to just the remaining of count
...
//
// EXERCISE (DONE): Write this function.
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	for ( off = 0; off < dir_oi->oi_size; off += OSPFS_DIRENTRY_SIZE ) {
		...
		// If inode is 0, then return empty directory found
		if ( od->od_ino == 0 )
			return od;
	}

	// No empty directories found, so add a new block to dir_oi
	r = add_block(dir_oi);
	if ( r < 0 )
		return ERR_PTR(r);

	ret_off = off;
	// Set all inode values of new directory entries to 0
	// 'off' already prepped at the right position from first for loop
	for ( ; off < dir_oi->oi_size; off += OSPFS_DIRENTRY_SIZE ) {
		od = ospfs_inode_data(dir_oi, off);
		od->od_ino = 0;
		od->od_name[0] = '\0';
	}

	// Return first new dir entry from added block
	od = ospfs_inode_data(dir_oi, ret_off);
	return od;
}
...
// find_empty_inode()
//   Finds an empty inode
//   Return 0 if no empty inodes found

uint32_t
find_empty_inode(void) {
	uint32_t new_ino;
	uint32_t ninodes = ospfs_super->os_ninodes;
	ospfs_inode_t *oi;

	// Look through inodes until we find inode with link count 0
	// Inodes 0 and 1 are reserved, so start at 2
	for ( new_ino = 2; new_ino < ninodes; new_ino++ ) {
		oi = ospfs_inode(new_ino);
		if ( oi && oi->oi_nlink == 0 )
			return new_ino;
	}

	// Didn't find any free inodes
	return 0;
}
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	// If directory with same name already exists, return -EEXIST
	if ( find_direntry(dir_oi, dentry->d_name.name, dentry->d_name.len) )
		return -EEXIST;

	// Create a blank direntry to use
	...
	// Initialize inode struct values
	oi = ospfs_inode(entry_ino);
	// If oi is NULL, return I/O error
	if ( !oi )
		return -EIO;
	oi->oi_size = 0;
	...
	// Initialize block pointers to all 0
	for ( i = 0; i < OSPFS_NDIRECT; i++ )
		oi->oi_direct[i] = 0;
	oi->oi_indirect = 0;
	oi->oi_indirect2 = 0;

	// Initialize direntry struct values
	od->od_ino = entry_ino;
	memcpy(od->od_name, dentry->d_name.name, dentry->d_name.len);
	// Append null byte to terminate name string
